...
└── src/
    ├── index.js
    ├── App.jsx                ← Composant principal (~2140 lignes, dont hook useSyncStatus)
...
- **Mode debug** 🔬 : modal depuis les Réglages, sections repliables, bouton "Tout copier".
...
**Fragilité connue (F1 audit)** : trois listes synchronisées manuellement — les states dans `buildAppState`, le tableau de deps du `useEffect`, et les sections de `DebugModal`. Tout nouvel état persisté doit être ajouté aux trois endroits. À consolider via `PERSISTED_KEYS` (Ω3, reporté).
...
- `ProgressionChart` : courbe élève (trait plein) + moyenne classe (pointillés), gestion trous (null)
- `ProgressionRadar` : radar multi-DS (max 8), bascule automatique vers courbe si n > 8
...
**Thèmes** : `"light"` (Cahier, Lora), `"dark"` (Ardoise, Lora), `"young"` (Lavande, Nunito). Chaque palette expose : `bg, card, border, surface, text, textMuted, textDim, accent, success, warning, danger, ruled, ruledLine, radius, headerFont, bodyFont, compColors { A, N, R, V }`. **`ruledLine` obligatoire** — son absence casse tout le CSS.
