...
- `RadarChart` : radar interactif complet (correction) — si le mode `exAbs`/`exRel` produit < 3 points, fallback silencieux vers `comp` via `effectiveMode`
...
| **AG** ✅    | App.jsx, AccueilTab.jsx                                                                           | Bugfix AccueilTab : boutons "Correction →", "Résultats", "Export" appellent désormais `setActiveExamId(lastExam.id)` avant `setMode(...)` — le dernier DS est sélectionné même si un autre DS était actif · prop `setActiveExamId` passée depuis App.jsx |
| **AH** ✅    | Charts.jsx                                                                                        | Bugfix RadarChart : clic vers mode `exAbs`/`exRel` sur un exam < 3 exercices → tableau trop court → composant disparaissait (`return null`). Correctif : si `rawData.length < 3 && mode !== "comp"`, fallback silencieux vers les données compétences (`effectiveMode = "comp"`) ; label en bas du radar utilise `effectiveMode` |
